...
class AgentB(BaseAgent):
    """
    Agent_C: Excel 数据筛选与填表。类名 AgentB 与编排入口 agent_b 保持一致。
...
def _project_root() -> Path:
    ...
    This file is located at: <root>/src/core/agents/agent_c.py
    """
...
def _load_data_module(path: Path):
    spec = importlib.util.spec_from_file_location("agent_c_api_data_module", str(path))
...
def run_agent_c_from_data_file(data_py: Path) -> Dict[str, Any]:
    """从 data.py 读取参数并调用 run_agent_c_api。"""
...
def run_agent_c_fill_from_entities(
    *,
    ...
    into one final template (xlsx/docx), reusing AgentC's mapping/filling logic.
    """
